...
import com.recording.trans.model.db.AppDatabase
...
object DBManager {
    ...
    fun getAllByDateDesc(context: Context): ArrayList<FileWithType> {
        val dao = AppDatabase.getDatabase(context).fileDao()
        val list = dao.findAllByDateDesc()
        if (list.isNullOrEmpty()) {
            return arrayListOf()
        }
    ...
    fun getAllByDateAsc(context: Context): ArrayList<FileWithType> {
        val dao = AppDatabase.getDatabase(context).fileDao()
        val list = dao.findAllByDateAsc()
        if (list.isNullOrEmpty()) {
            return arrayListOf()
        }
    ...
    fun getAllBySizeDesc(context: Context): ArrayList<FileWithType> {
        val dao = AppDatabase.getDatabase(context).fileDao()
        val list = dao.findAllBySizeDesc()
        if (list.isNullOrEmpty()) {
            return arrayListOf()
        }
    ...
    fun getAllBySizeAsc(context: Context): ArrayList<FileWithType> {
        val dao = AppDatabase.getDatabase(context).fileDao()
        val list = dao.findAllBySizeAsc()
        if (list.isNullOrEmpty()) {
            return arrayListOf()
        }
    ...
    fun getAudioByKey(context: Context, sort: String, minSize: Long, maxSize: Long, minDate: Long, maxDate: Long): ArrayList<FileWithType> {
        val dao = AppDatabase.getDatabase(context).fileDao()
        val result = arrayListOf<FileWithType>()
        JLog.i("$sort+$minSize+$maxSize+$minDate+$maxDate")
        when (sort) {
            "default" -> {
                val list = dao.findAudio(minSize, maxSize, minDate, maxDate) as ArrayList<FileWithType>
                if (list.isNotEmpty()) {
                    result.addAll(list)
                }
            }
            ...
            "date_desc" -> {
                val list = dao.findAudioByDateDesc(minSize, maxSize, minDate, maxDate) as ArrayList<FileWithType>
                if (list.isNotEmpty()) {
                    result.addAll(list)
                }
            }
            ...
            "date_asc" -> {
                val list = dao.findAudioByDateAsc(minSize, maxSize, minDate, maxDate) as ArrayList<FileWithType>
                if (list.isNotEmpty()) {
                    result.addAll(list)
                }
            }
            ...
            "size_desc" -> {
                val list = dao.findAudioBySizeDesc(minSize, maxSize, minDate, maxDate) as ArrayList<FileWithType>
                if (list.isNotEmpty()) {
                    result.addAll(list)
                }
            }
            ...
            "size_asc" -> {
                val list = dao.findAudioBySizeAsc(minSize, maxSize, minDate, maxDate) as ArrayList<FileWithType>
                if (list.isNotEmpty()) {
                    result.addAll(list)
                }
            }
    ...
    fun update(context: Context, file: FileWithType) {
        val dao = AppDatabase.getDatabase(context).fileDao()
        val acc = dao.find(file.path)
        if (acc != null) {
            dao.update(file)
