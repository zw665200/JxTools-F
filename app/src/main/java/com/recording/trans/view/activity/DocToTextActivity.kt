...
class DocToTextActivity : BaseActivity() {
    ...
    override fun initData() {
        LogReportManager.logReport("文档转文字", "访问页面", LogReportManager.LogType.OPERATION)
    }

    private fun choosePic(index: Int) {
        when (index) {
            0 -> content.text = getString(R.string.doc_to_text_01)
            1 -> content.text = getString(R.string.doc_to_text_02)
        }
    }
    ...
    private fun doTask() {
        if (imageUri == null) {
            ToastUtil.showShort(this, "请导入pdf文件")
            return
        }

        PayManager.get().checkPay(this) {
            if (it) {
                LogReportManager.logReport("文档转文字", "使用功能", LogReportManager.LogType.OPERATION)
                ...
                if (realPath != null) {
                    launch(Dispatchers.IO) {
                        AudioManager.get().getTextFromDoc(realPath, object : HttpCallback {
                            override fun onSuccess() {
                                runOnUiThread {
                                    val intent = Intent(this@DocToTextActivity, TextToVoiceActivity::class.java)
                            ...
                            override fun onFailed(msg: String) {
                                runOnUiThread {
                                    ...
                                    ToastUtil.showShort(this@DocToTextActivity, msg)
                                }
                            }
                        })
                    }
    ...
    private fun checkFileSize(uri: Uri) {
        ...
        if (length > 3 * 1024 * 1024) {
            ToastUtil.show(this, "上传文档大小不能超过3MB")
        } else {
            val realPath = FileUtil.getPathFromUri(this, uri)
            if (realPath != null) {
                if (realPath.endsWith(".pdf")) {
                    imageUri = uri
                    import.text = "已选择文档"
                } else {
                    ToastUtil.showShort(this, "目前只支持pdf文档")
                }
