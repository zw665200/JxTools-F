...
public class VoiceWaveView extends View {
    ...
    private void init(AttributeSet attrs, int defStyle) {
        final TypedArray a = getContext().obtainStyledAttributes(
                attrs, R.styleable.VoiceWaveView, defStyle, 0);
        mWaveColor = a.getColor(
                R.styleable.VoiceWaveView_waveColor,
                mWaveColor);
        mBaseLineColor = a.getColor(
                R.styleable.VoiceWaveView_baselineColor,
                mBaseLineColor);

        waveStrokeWidth = a.getDimension(
                R.styleable.VoiceWaveView_waveStokeWidth,
                waveStrokeWidth);

        max = (short) a.getInt(R.styleable.VoiceWaveView_maxValue, max);
        invalidateTime = a.getInt(R.styleable.VoiceWaveView_invalidateTime, invalidateTime);

        space = a.getDimension(R.styleable.VoiceWaveView_space, space);
        a.recycle();
        initPainters();
