...
import com.recording.trans.callback.VideoCallback
...
class VideoFormatTransActivity : BaseActivity() {
    ...
    override fun initData() {
        ...
        LogReportManager.logReport("视频格式转换", "访问页面", LogReportManager.LogType.OPERATION)
    }
            ...
            .addBindView { itemView, itemData ->
                ...
                itemView.voice_select.setOnClickListener {
                    PayManager.get().checkPay(this) {
                        if (it) {
                            VideoFormatTransDialog(this, itemData.path).show()
                        } else {
    ...
    private fun sortAllSelectedList() {
        ...
        launch(Dispatchers.IO) {
            sortVoices = DBManager.getVoiceByKey(this@VideoFormatTransActivity, type, minSize, maxSize, minDate, maxDate)
            launch(Dispatchers.Main) {
                mainVoices.clear()
                if (sortVoices.size > 0) {
                    mainVoices.addAll(sortVoices)

                    val tText = getString(R.string.video_list_title) + "(${sortVoices.size})"
                    title.text = tText
                } else {
                    title.text = getString(R.string.video_list_title)
                }
        ...
        val callback = object : VideoCallback {
            ...
            override fun onProgress(file: FileWithType) {
                launch(Dispatchers.Main) {
                    ...
                    title.text = getString(R.string.video_list_title) + "(${sortVoices.size})"
                }
            }
            ...
            override fun onFailed(message: String) {
                launch(Dispatchers.Main) {
                    ...
                    ToastUtil.showShort(this@VideoFormatTransActivity, message)
                }
            }
        }

        thread { WxManager.getInstance(this@VideoFormatTransActivity).getVideos(this, callback) }
    }
