 ...
 */
class VoiceFormatTransDialog(activity: Activity, filePath: String) : Dialog(activity, R.style.app_dialog), CoroutineScope by MainScope() {
    ...
    private fun initView() {
        val view = LayoutInflater.from(mContext).inflate(R.layout.p_voice_format_trans, null)
        setContentView(view)
        setCancelable(false)

        type = findViewById(R.id.audio_type)
    ...
    private fun convert(format: String) {
        LogReportManager.logReport("音频格式转换", "使用功能", LogReportManager.LogType.OPERATION)
        converter.text = "转换中..."
    ...
    private fun doTask(path: String, format: String) {
        ...
        launch(Dispatchers.IO) {
            AudioManager.get().audioTrans(mContext, path, format, callback)
        }
