...
//                edittext_layout.setBackgroundResource(R.drawable.hd_input_bar_bg_normal);
        ...
        buttonPressToSpeak.setAudioFinishRecorderListener((seconds, filePath) -> {
            if (listener != null) {
                listener.onRecorderCompleted(seconds, filePath);
            }
        });

        buttonSend.setOnClickListener(v -> sendTextMsg());
    ...
    private boolean sendTextMsg() {
        if (listener != null) {
            ...
            if (Dict.INSTANCE.shieldSensitiveWords(s)) {
                ToastHelper.show(context, "您发送的内容包含敏感词，无法发送");
            } else {
                editText.setText("");
                refleshEmojiSendBtn();
